...
class CompleteBasicCleaningPaymentUseCase(private val repository: CleaningServiceRepository) {
    suspend operator fun invoke(request: CompleteBasicCleaningPaymentRequest): Flow<UseCaseResult<PaymentResponseModel>> {
        return repository.completeBasicCleaningPayment(request.toDto()).map { result ->
            when (result) {
                is Result.Success -> {
