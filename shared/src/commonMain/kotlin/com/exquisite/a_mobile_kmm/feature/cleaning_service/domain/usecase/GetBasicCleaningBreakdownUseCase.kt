...
import com.exquisite.a_mobile_kmm.feature.cleaning_service.data.mapper.toBasicCleaningBreakdownModel
...
import com.exquisite.a_mobile_kmm.feature.cleaning_service.domain.model.GetBasicCleaningBreakdownRequest
...
class GetBasicCleaningBreakdownUseCase(private val repository: CleaningServiceRepository) {
    suspend operator fun invoke(request: GetBasicCleaningBreakdownRequest): Flow<UseCaseResult<BasicCleaningBreakdownModel>> {
        return repository.getBasicCleaningBreakdown(request.toDto()).map { result ->
            when (result) {
                is Result.Success -> {
                    if (result.data.responseCode == "00") {
                        val model = result.data.toBasicCleaningBreakdownModel()
                        if (model != null) {
