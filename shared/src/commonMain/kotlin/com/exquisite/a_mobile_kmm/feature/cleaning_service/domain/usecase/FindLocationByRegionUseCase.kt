...
import com.exquisite.a_mobile_kmm.feature.cleaning_service.data.mapper.toLocationModelList
...
class FindLocationByRegionUseCase(private val repository: CleaningServiceRepository) {
    suspend operator fun invoke(regionId: Int): Flow<UseCaseResult<List<LocationModel>>> {
        return repository.findLocationByRegion(regionId).map { result ->
            when (result) {
                is Result.Success -> {
                    if (result.data.responseCode == "00" && result.data.data != null) {
                        val model = result.data.toLocationModelList()
                        if (model != null) {
