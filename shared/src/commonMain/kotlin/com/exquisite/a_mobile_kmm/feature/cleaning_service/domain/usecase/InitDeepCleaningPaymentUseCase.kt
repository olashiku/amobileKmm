...
class InitDeepCleaningPaymentUseCase(private val repository: CleaningServiceRepository) {
    suspend operator fun invoke(request: InitDeepCleaningPaymentRequestDto): Flow<UseCaseResult<PaymentModel>> {
        return repository.initDeepCleaningPayment(request).map { result ->
            when (result) {
                is Result.Success -> {
