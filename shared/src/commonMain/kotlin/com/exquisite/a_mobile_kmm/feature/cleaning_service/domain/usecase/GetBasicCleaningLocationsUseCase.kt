...
import com.exquisite.a_mobile_kmm.feature.cleaning_service.data.mapper.toBasicCleaningLocationModelList
...
class GetBasicCleaningLocationsUseCase(private val repository: CleaningServiceRepository) {
    suspend operator fun invoke(): Flow<UseCaseResult<List<BasicCleaningLocationModel>>> {
        return repository.getBasicCleaningLocations().map { result ->
            when (result) {
                is Result.Success -> {
                    if (result.data.responseCode == "00" && result.data.data != null) {
                        val model = result.data.toBasicCleaningLocationModelList()
                        if (model != null) {
