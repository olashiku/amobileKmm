...
import com.exquisite.a_mobile_kmm.feature.cleaning_service.data.mapper.toCleaningTypeModelList
...
class FindCleaningTypeUseCase(private val repository: CleaningServiceRepository) {
    suspend operator fun invoke(): Flow<UseCaseResult<List<CleaningTypeModel>>> {
        return repository.findCleaningType().map { result ->
            when (result) {
                is Result.Success -> {
                    if (result.data.responseCode == "00" && result.data.data != null) {
                        val model = result.data.toCleaningTypeModelList()
                        if (model != null) {
