...
import com.exquisite.a_mobile_kmm.feature.mobile_toilet.data.mapper.toStandardToiletModelList
...
class GetStandardToiletsListUseCase(private val repository: MobileToiletRepository) {
    suspend operator fun invoke(): Flow<UseCaseResult<List<StandardToiletModel>>> {
        return repository.getStandardToiletsList().map { result ->
            when (result) {
                is Result.Success -> {
                    if (result.data.responseCode == "00" && result.data.data != null) {
                        val model = result.data.toStandardToiletModelList()
                        if (model != null) {
