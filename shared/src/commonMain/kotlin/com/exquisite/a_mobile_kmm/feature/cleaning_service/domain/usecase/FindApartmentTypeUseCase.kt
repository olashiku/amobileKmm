...
import com.exquisite.a_mobile_kmm.feature.cleaning_service.data.mapper.toApartmentTypeModelList
...
class FindApartmentTypeUseCase(private val repository: CleaningServiceRepository) {
    suspend operator fun invoke(): Flow<UseCaseResult<List<ApartmentTypeModel>>> {
        return repository.findApartmentType().map { result ->
            when (result) {
                is Result.Success -> {
                    if (result.data.responseCode == "00" && result.data.data != null) {
                        val model = result.data.toApartmentTypeModelList()
                        if (model != null) {
