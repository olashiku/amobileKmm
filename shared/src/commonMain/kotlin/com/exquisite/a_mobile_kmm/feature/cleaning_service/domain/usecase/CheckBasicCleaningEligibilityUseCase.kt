...
class CheckBasicCleaningEligibilityUseCase(
    private val repository: CleaningServiceRepository
) {
    suspend operator fun invoke(customerId: Int): Flow<UseCaseResult<Boolean>> {
        return repository.checkBasicCleaningEligibility(customerId).map { result ->
            when (result) {
                is Result.Success -> {
                    if (result.data.responseCode == "00") {
                        UseCaseResult.Success(result.data.data)
                    } else {
