...
import com.exquisite.a_mobile_kmm.feature.mobile_toilet.data.mapper.toEventTypeModelList
...
class GetEventTypeUseCase(private val repository: MobileToiletRepository) {
    suspend operator fun invoke(): Flow<UseCaseResult<List<EventTypeModel>>> {
        return repository.getEventType().map { result ->
            when (result) {
                is Result.Success -> {
                    if (result.data.responseCode == "00" && result.data.data != null) {
                        val model = result.data.toEventTypeModelList()
                        if (model != null) {
