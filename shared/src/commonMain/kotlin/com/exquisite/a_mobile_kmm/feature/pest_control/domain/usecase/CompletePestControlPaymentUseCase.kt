...
class CompletePestControlPaymentUseCase(private val repository: PestControlRepository) {
    suspend operator fun invoke(request: CompletePestControlPaymentRequestDto): Flow<UseCaseResult<PestControlResponseModel>> {
        return repository.completePayment(request).map { result ->
            when (result) {
                is Result.Success -> {
