...
class CompletePestControlPaymentUseCase(private val repository: PestControlRepository) {
    suspend operator fun invoke(request: CompletePestControlPaymentRequest): Flow<UseCaseResult<PestControlResponseModel>> {
        return repository.completePayment(request.toDto()).map { result ->
            when (result) {
                is Result.Success -> {
