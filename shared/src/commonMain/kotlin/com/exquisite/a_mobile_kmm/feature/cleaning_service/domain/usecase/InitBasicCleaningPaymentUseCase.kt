...
class InitBasicCleaningPaymentUseCase(private val repository: CleaningServiceRepository) {
    suspend operator fun invoke(request: InitBasicCleaningPaymentRequest): Flow<UseCaseResult<PaymentModel>> {
        return repository.initBasicCleaningPayment(request.toDto()).map { result ->
            when (result) {
                is Result.Success -> {
