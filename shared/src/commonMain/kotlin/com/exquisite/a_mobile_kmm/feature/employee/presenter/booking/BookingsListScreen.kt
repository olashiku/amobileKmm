...
fun BookingsListScreen(
    ...
    val bookingsState = viewModel.bookingsListState.collectAsStateWithLifecycle()
    val currentBookingType = viewModel.currentBookingType.collectAsStateWithLifecycle()

    val isLoadingMore by viewModel.isLoadingMore.collectAsStateWithLifecycle()
    val listState = rememberLazyListState()

    var isLoading by remember { mutableStateOf(false) }
    var customerBookingsModel by remember { mutableStateOf(emptyList<AgentBooking>()) }

    // Initial load on first composition
    LaunchedEffect(Unit) {
        viewModel.fetchBookings(tag.uppercase())
    }

    when (val result = bookingsState.value) {
        is BookingsListUiState.Success -> {
            isLoading = false
            println("customerBookingsModel $customerBookingsModel")
            customerBookingsModel = result.bookings
        }

        is BookingsListUiState.Error -> {
            isLoading = false
        }

        is BookingsListUiState.Loading -> {
            isLoading = true
        }

        is BookingsListUiState.Initial -> {}
    }
    ...
    Box(
        ...
        Column{
            ...
            LazyColumn(
                ...
                if (isLoading && customerBookingsModel.isEmpty()) {
                    items(5) {
                        BookingItemSkeleton()
                    }
                } else if (!isLoading && customerBookingsModel.isEmpty()) {
                    // Empty state
                    item {
                        Column(
                            modifier = Modifier
                    ...
                    items(customerBookingsModel) { item ->
